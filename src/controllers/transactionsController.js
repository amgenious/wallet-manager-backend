...
export async function getTransactionsByUserId(req,res){
        ...
        SELECT * FROM transactions WHERE user_id = ${userId} ORDER BY
        created_at DESC
        `
...
export async function createTransaction(req,res){
        try{
            ...
            if(!title ||!amount||!category||!user_id === undefined){
                return res.status(400).json({mesdage:"All fields are required"})
            }
...
export async function getTransactionSummary(req,res){
   ...
   try{
    ...
    SELECT COALESCE(SUM(amount),0) as balance FROM transactions WHERE
    user_id = ${userId}
    `
